...
class ResNetLayers(link.Chain):
    ...
    def functions(self):
        return collections.OrderedDict([
            ...
            ('res4', [self.res4]),
            ('res5', [self.res5]),
            ('pool5', [global_average_pooling_2d]),
            ('fc6', [self.fc6]),
            ('prob', [softmax]),
        ])
    ...
    def convert_caffemodel_to_npz(cls, path_caffemodel, path_npz, n_layers=50):
        ...
        if n_layers == 50:
            transfer_resnet50(caffemodel, chainermodel)
        elif n_layers == 101:
            transfer_resnet101(caffemodel, chainermodel)
        elif n_layers == 152:
            transfer_resnet152(caffemodel, chainermodel)
        else:
            raise ValueError('The n_layers argument should be either 50, 101,'
                             ' or 152, but {} was given.'.format(n_layers))
...
def transfer_block(src, dst, names):
...
def transfer_resnet101(src, dst):
    dst.conv1.W.data[:] = src.conv1.W.data
    dst.bn1.avg_mean[:] = src.bn_conv1.avg_mean
    dst.bn1.avg_var[:] = src.bn_conv1.avg_var
    dst.bn1.gamma.data[:] = src.scale_conv1.W.data
...
def transfer_resnet152(src, dst):
    dst.conv1.W.data[:] = src.conv1.W.data
    dst.bn1.avg_mean[:] = src.bn_conv1.avg_mean
    dst.bn1.avg_var[:] = src.bn_conv1.avg_var
    dst.bn1.gamma.data[:] = src.scale_conv1.W.data
