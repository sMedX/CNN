#ifndef __agtkCommandLineArgumentParser_h
#define __agtkCommandLineArgumentParser_h
 ...
 *   bool returnValue1 = parser->GetValue( "-key1", arguments1 );
 *
 *   float argument2;
 *   bool returnValue2 = parser->GetValue( "-key2", argument2 );
 *
...
class AGTKCore_EXPORT CommandLineArgumentParser : public itk::Object
{
public:
  ...
  itkTypeMacro(CommandLineArgumentParser, itk::Object);
  ...
  template <class T>
  bool GetValue(const std::string& key, T& arg)
  {
    std::vector<T> vec;
    bool returnvalue = this->GetValue(key, vec);
  ...
  /** Get command line itk argument. */
  template <class T, class TValue>
  bool GetITKValue(const std::string& key, T& arg)
  {
    std::vector<TValue> vec(T::Dimension);

    for (int n = 0; n < T::Dimension; ++n) { vec[n] = arg[n]; }

    bool returnvalue = this->GetValue<TValue>(key, vec);

    for (int n = 0; n < T::Dimension; ++n) { arg[n] = vec[n]; }
  ...
  bool GetITKValue(const std::string& key, T& arg)
  {
    bool returnvalue = this->GetITKValue<T, double>(key, arg);
...
#endif // __agtkCommandLineArgumentParser_h
